...
class RevEcho(nn.Module):
    ...
    def __init__(self, proba=0.5, initial=0.3, rt60=(0.3, 1.3), first_delay=(0.01, 0.03),
                 repeat=3, jitter=0.1, reverb_factor=0.2, sample_rate=16000):
        super().__init__()
        self.proba = proba
    ...
    def forward(self, wav):
        ...
        reverb = self._reverb(wav, initial, first_delay, rt60)
        wav += self.reverb_factor * reverb
...
class BandMask(nn.Module):
    ...
    def __init__(self, maxwidth=0.2, bands=120, sample_rate=8_000):
        """__init__.
